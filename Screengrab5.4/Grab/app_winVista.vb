...
Module app_winVista
  ...
  Public Function GDIRectangle(ByVal hdc As IntPtr, ByVal X1 As Integer, ByVal Y1 As Integer, ByVal X2 As Integer, ByVal Y2 As Integer) As Boolean
  End Function
  <System.Runtime.InteropServices.DllImportAttribute("gdi32.dll")> _
  ...
  Sub makeFormGlassReady(ByVal frm As Form, ByVal ctrl As Control, ByVal ctrlPos As DockStyle)
    If checkIfGlassEnabled() Then
      'hideTitleBarContents(frm.Handle)
      ctrl.BackColor = Color.Black
      'tbrZoom.BackColor = Color.Black
      If ctrlPos = DockStyle.Top Then
        AddGlassToWin(frm.Handle, 0, 0, ctrl.Height, 0)
        '  ctrl.Top = -3 : ctrl.Height += 3
      ElseIf ctrlPos = DockStyle.Left Then
        AddGlassToWin(frm.Handle, ctrl.Width, 0, 0, 0)
        ' ctrl.Left = -3 : ctrl.Width += 3
      End If
      ownerDrawSettings.Add(frm, New Dictionary(Of String, Object))
      For Each sctrl As Control In ctrl.Controls
        If TypeOf sctrl Is Button Or TypeOf sctrl Is CheckBox Or TypeOf sctrl Is Label Then
          AddHandler sctrl.Paint, AddressOf ownerDrawForGlass
          ownerDrawSettings(frm).Add(sctrl.Name, sctrl.Text)
          sctrl.Text = ""
          'ctrl.BackColor = Color.Black
        End If
        If TypeOf sctrl Is ComboBox Or TypeOf sctrl Is TextBox Or TypeOf sctrl Is ListBox Then
          Debug.Print("GlassWin: " + sctrl.Name)
  ...
  Sub ownerDrawForGlass(ByVal sender As Object, ByVal e As System.Windows.Forms.PaintEventArgs)
    ...
    Dim txt = ownerDrawSettings(sender.findform)(sender.Name)
    ...
    If TypeOf sender Is CheckBox Then
      Dim rect As New Rectangle(10, 2, sender.Width - 10, sender.height - 4)
      Dim stat = VisualStyles.CheckBoxState.UncheckedNormal
      If sender.Checked Then stat = VisualStyles.CheckBoxState.CheckedNormal

      DrawTextGlow(e.Graphics, txt, sender.Font, rect, Color.Black, TextFormatFlags.VerticalCenter Or TextFormatFlags.HorizontalCenter Or TextFormatFlags.NoPrefix Or TextFormatFlags.SingleLine)

      CheckBoxRenderer.DrawCheckBox(e.Graphics, New Point(0, sender.height / 2 - 7), stat)
    ElseIf TypeOf sender Is Button Then
      e.Graphics.DrawString(txt, sender.font, Brushes.Black, xpos + sender.padding.left, ypos + sender.padding.top, strf)
  ...
  Sub drawRectOnHDC(ByVal hdc As Integer, ByVal rect As Rectangle, ByVal fillColor As Color)
    ...
    GDIRectangle(hdc, rect.X, rect.Y, rect.Right, rect.Bottom)
  ...
  Public Class TextBoxGlassDrawer
    ...
    Protected Overloads Overrides Sub WndProc(ByRef m As Message)
      ...
      If m.Msg = WindowsMessages.WM_PAINT Then
        ...
        drawRectOnHDC(BufferedDC, ctrl.ClientRectangle, Color.Black)

        BufferedPaintSetAlphaRect(BuffDCHandle, New RECT(0, 0, ctrl.Width, ctrl.Height), 255)
