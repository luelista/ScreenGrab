...
nochmal_versuchen:
    If frm.ShowDialog = Windows.Forms.DialogResult.OK Then
      If doLogin(frm.UsernameTextBox.Text, frm.PasswordTextBox.Text) Then
        Return True
      Else
        frm.PasswordTextBox.Text = "" : frm.PasswordTextBox.Focus()
        GoTo nochmal_versuchen
      End If
    Else
      Return False
    End If
  ...
  Function doLogin(ByVal userName As String, ByVal passWord As String) As Boolean
    ...
    If lines(0) = "Login OK" Then
      ...
      twSessID = lines(2)
      'frm_main.ToolStripStatusLabel1.Text = twSessID
  ...
  Sub upload_file(ByVal sourceFilespec As String, ByVal sc_name As String, ByVal use_temp As Boolean, ByVal fs_team As String, ByVal fs_dir As String, ByRef errMes As String, Optional ByVal sc_ext As String = "")
    ...
    Try
      ...
      Dim fieldPart As String = _
        ...
        IO.File.ReadAllText(sourceFilespec) & vbCrLf & _
      "--" & strBoundary & "--"
      'WICHTIG: Es müssen zwei CRLF sein -- sonst schneidet er den Anfang der Datei ab
      ...
      While xmlhttp.ReadyState <> 4
        TwAjax.idle()
        If timer > 1000 Then xmlhttp = Nothing : errMes = "ERROR, Timeout"
        timer += 1
      End While
